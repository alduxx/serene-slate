...
const PoesiasList = () => {
  ...
  if (loading) {
    return <section id="poesias" className="py-24 bg-secondary/30">
        <div className="container mx-auto px-6">
          <div className="text-center">Carregando poesias...</div>
        </div>
      </section>;
  }
  return <section id="poesias" className="py-24 relative overflow-hidden" style={{
    ...
    backgroundBlendMode: 'overlay'
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="max-w-3xl mb-16 animate-fade-in">
          ...
          <h2 className="font-display text-5xl leading-tight mb-4">Poesias</h2>
          <p className="text-lg text-muted-foreground leading-relaxed">Versos que sobrevivem e fazem viver, apesar do tempo</p>
        </div>

        {/* Poetry Grid */}
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {poesias.map((poesia, index) => <Card key={poesia.slug} className="hover:shadow-xl transition-all duration-300 bg-card/95 backdrop-blur border-2 animate-fade-in group" style={{
          animationDelay: `${index * 100}ms`
              ...
              <CardHeader>
                ...
                {poesia.frontmatter.excerpt && <Link to={`/poesias/${poesia.slug}`}>
              ...
              <CardContent>
                <Link to={`/poesias/${poesia.slug}`}>
                  <Button variant="ghost" className="w-full justify-start text-primary hover:text-primary/80 hover:bg-transparent p-0">
                    Ler poesia →
                  </Button>
                </Link>
              </CardContent>
